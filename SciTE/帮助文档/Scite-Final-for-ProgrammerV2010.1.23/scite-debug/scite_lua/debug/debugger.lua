...
function scite_ReplaceLine(pane,l,txt)
    pane.TargetStart = pane:PositionFromLine(l)
...
scite_OnOutputLine (function (line)
    ...
    if not lines then
        ...
        scite_OnKey(function(key)
            ...
            local delta
            -- hack: for these keys, accounts for the offset for GDK_Up, etc.
            if GTK then key = key - 65324 end
            if key == 38 then
            ...
            if delta then
                ...
                if txt then
                    local l = output:LineFromPosition(output.CurrentPos)
...
function Dbg.discriminator(target)
    return target:find('^:') == nil
end
...
--~   	 print('*',line)
    ...
    if file and status ~= 'dead' then
        -- this is a useful way to ignore some output...
