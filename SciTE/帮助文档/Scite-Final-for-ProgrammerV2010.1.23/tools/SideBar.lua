...
print 'Scite/Lua printed by print ' 
...
if tonumber(props['sidebar.show'])==1 then
	if win then
		...
		gui.set_panel(win_parent,"left")  -- show sidebar on left, comment by David Lv
	end
end
...
local function Functions_GetNames()
	...
	if( lang == nil  ) then  -- added by david
		lang = "perl"
	end
	-- 2 lines added by david lv, to test append text to output :
	
	--output:AppendText(lang..editor.LexerLanguage.."\n")
...
function SideBar_ShowHide()
	if tonumber(props['sidebar.show'])==1 then
		if win then			
			...
			-- 2 lines added by david lv, to test append text to output :
			output:AppendText('Scite/Lua hide\n')
			output:GotoPos(output.Length)
			gui.set_panel()
		end
		...
		-- 2lines added by david lv, to test append text to output :
		output:AppendText('Scite/Lua show\n')
		output:GotoPos(output.Length)
...
function OnOpen(file)
	-- 2 lines added by david lv, to test append text to output :
	output:AppendText('Scite/Lua OnOpen(file)\n')
	output:GotoPos(output.Length)
...
function OnSave(file)
	-- 2 lines added by david lv, to test append text to output :
	output:AppendText('Scite/Lua OnSave called in SideBar.lua \n')
	output:GotoPos(output.Length)
